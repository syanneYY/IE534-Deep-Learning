...
X = X_batch.mean(dim=0).type(torch.float32)
X = X.repeat(10,1,1,1)
...
# The gradient is calculated and the original image X is modified by this gradient in order to maximize the output.
lr = 0.1
weight_decay = 0.001
for i in range(200):
    _, output = model(X)
