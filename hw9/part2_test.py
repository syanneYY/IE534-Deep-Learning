...
prediction_directory = '3d/'
if not os.path.exists(prediction_directory):
    os.makedirs(prediction_directory)
for label in class_list:
    if not os.path.exists(prediction_directory+label+'/'):
        os.makedirs(prediction_directory+label+'/')
# acc_top1 = 0.0 top_10 accuracy simply signifies how often the correct class is any of the top 10 predicted classes.
# The top_10 accuracy simply signifies how often the correct class is any of the top 10 predicted classes.
# Create some variables to track the (top 1,top 5,top 10) accuracy.
acc_top1 = 0.0
acc_top5 = 0.0
acc_top10 = 0.0
...
for i in range(len(test[0])):
    ...
    data = np.zeros((nFrames,3,sequence_length, IMAGE_SIZE,IMAGE_SIZE),dtype=np.float32)
    sequence = nFrames//sequence_length
    for j in range(sequence):
        for l in range(sequence_length*j, sequence_length*(j+1)):
            frame = h['video'][l]
            ...
            data[j,:,l - sequence_length*j,:,:] = frame
    h.close()
    prediction = np.zeros((sequence, NUM_CLASSES), dtype=np.float32)

    loop_i = list(range(0, sequence, 5))
    loop_i.append(sequence)
    ...
    for j in range(len(loop_i) - 1):
        ...
        prediction[loop_i[j]:loop_i[j + 1]] = output.data.cpu().numpy()
        # prediction[loop_i[j]:loop_i[j + 1]] = prediction[loop_i[j]:loop_i[j + 1]].detach().numpy()
        # detach()
